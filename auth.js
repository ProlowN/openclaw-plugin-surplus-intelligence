...
// Seller keys minted this session, scoped by API base so a key issued for one
...
async function getSellerAuth(ctx = {}) {
  ...
  const cached = mintedSellerKeys.get(config.getApiBase(ctx))
...
async function mintSellerKey(ctx = {}) {
  ...
  const challengeResp = await fetch(
    `${apiBase}${config.SELLER_AUTH_CHALLENGE}?address=${encodeURIComponent(wallet.address)}`,
  )
  ...
  const keyResp = await fetch(`${apiBase}${config.SELLER_AUTH_KEY}`, {
    method: 'POST',
  ...
  mintedSellerKeys.set(apiBase, auth)
  return auth
